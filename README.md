...
- Code Interpreter used to run (and hence evaluate) the Python code generated by the LLM.
...
When a LLM lacks built-in support for function calling, we can still achieve the same result. In this approach, we instruct Llama 3.1 to return just plain markdown code blocks that are "manually" parsed for a code interpreter to process. 
